...
export const TraceViewerEnhanced: React.FC<TraceViewerProps> = ({ spans, colorByService = true }) => {
  ...
  const [animatingSpans, setAnimatingSpans] = useState<Set<string>>(new Set())
  ...
  const toggleExpand = useCallback(
    (spanId: string) => {
      const isExpanding = !expandedSpans.has(spanId)
      const span = spanMap.get(spanId)

      if (!span || span.children.length === 0) return

      if (isExpanding) {
        // First update the expanded state to add rows to the DOM
        setExpandedSpans((prev) => new Set([...prev, spanId]))

        // Then after a small delay to ensure DOM update, mark rows for animation
        setTimeout(() => {
          // Get all direct children of this span
          const childSpanIds = span.children.map((child) => child.span_id)

          // Mark these children as animating (entering)
          setAnimatingSpans((prev) => {
            const newSet = new Set(prev)
            childSpanIds.forEach((id) => newSet.add(`entering-${id}`))
            return newSet
          })
          ...
          setTimeout(() => {
            setAnimatingSpans((prev) => {
              const newSet = new Set(prev)
              childSpanIds.forEach((id) => newSet.delete(`entering-${id}`))
              return newSet
            })
          }, 300)
        }, 10)
      } else {
        // For collapsing, first mark children for exit animation
        const childSpanIds = span.children.map((child) => child.span_id)

        // Mark direct children as exiting
        setAnimatingSpans((prev) => {
          const newSet = new Set(prev)
          childSpanIds.forEach((id) => newSet.add(`exiting-${id}`))
          return newSet
        })

        // After animation duration, update the expanded state to remove from DOM
        setTimeout(() => {
          ...
          setAnimatingSpans((prev) => {
            const newSet = new Set(prev)
            childSpanIds.forEach((id) => newSet.delete(`exiting-${id}`))
            return newSet
          })
        }, 300)
      }
    },
    [expandedSpans, spanMap],
  )
        ...
        <table className="w-full border-collapse">
          ...
          <tbody>
            {visibleSpans.map((span) => {
              ...
              const isAnimating = animatingSpans.has(span.span_id)
              ...
              return (
                <tr
                  ...
                  className={`border-t border-gray-200 hover:bg-gray-50 trace-row 
  ${isSelected ? "bg-blue-50" : ""} 
  ${isFocused ? "outline outline-2 outline-primary" : ""}
  ${animatingSpans.has(`entering-${span.span_id}`) ? "trace-row-entering" : ""}
  ${animatingSpans.has(`exiting-${span.span_id}`) ? "trace-row-exiting" : ""}`}
                  onClick={() => {
                ...
                >
                    ...
                    <div className="flex items-center">
                      ...
                      {hasChildren ? (
                        <button
                          ...
                          className={`mr-1 p-1 rounded-sm hover:bg-gray-200 ${isAnimating ? "animate-pulse" : ""}`}
                        ...
                        >
                          {isExpanded ? (
                            <ChevronDown
                              size={14}
                              className={`transition-transform duration-300 ${isAnimating ? "rotate-90" : ""}`}
                            />
                          ) : (
                            <ChevronRight
                              size={14}
                  ...
                  <td className="px-4 py-2 relative">
                    <div className="h-6 w-full relative">
                      ...
                      <div
                        className={`absolute top-1 h-4 ${barColor} rounded-sm z-10 waterfall-bar`}
                        style={{
                          left: `${startPosition}%`,
                          width: `${Math.max(0.5, widthPercentage)}%`,
                          opacity: isSelected ? 1 : 0.7,
                          transition: isAnimating ? "all 0.3s ease" : "",
                        }}
