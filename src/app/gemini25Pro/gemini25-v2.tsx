...
// Helper function processSpans (remains the same - included for completeness)
...
const TimeMarkers = ({ totalDuration }) => {
    ...
    if (totalDuration <= 0) {
        return <div className="text-center text-xs text-gray-500 dark:text-gray-400">0ms</div>;
    }
    ...
    return (
        <div className="relative h-4 w-full"> {/* Container for markers */}
            {markers.map((marker, index) => (
                ...
                >
                    {/* Tick mark */}
                    <div className="h-1/2 w-px bg-gray-400 dark:bg-gray-500"></div>
                    {/* Label - centered using transform, adjust last label */}
                    <span
                        className={`absolute top-full mt-px text-xs text-gray-500 dark:text-gray-400 whitespace-nowrap ${
                            index === markers.length - 1 ? 'right-0 translate-x-0' : '-translate-x-1/2' // Position last label differently
                        } ${
                            index === 0 ? 'left-0 -translate-x-0' : '' // Position first label differently
                        }`}
...
// --- Recursive Row Component (Tailwind - Widths Adjusted) ---
const SpanRow = ({
    span,
    depth,
    traceStartTime,
...
// --- Main Table Component (Tailwind - Widths Adjusted & Time Markers) ---
        ...
        <div className="overflow-x-auto">
            <table className="w-full border-collapse table-fixed text-sm font-sans dark:text-slate-300">
                <thead className="bg-gray-100 dark:bg-slate-800">
                    <tr>
                        {/* Adjusted Width */}
                        <th className="border border-gray-200 dark:border-slate-600 p-2 font-bold text-left w-[30%]">Name</th>
                        {/* Adjusted Width */}
                        <th className="border border-gray-200 dark:border-slate-600 p-2 font-bold text-right w-[10%]">Duration</th>
                        {/* Adjusted Width & Content */}
                        <th className="border border-gray-200 dark:border-slate-600 p-2 font-bold text-left w-[60%] align-bottom"> {/* Align bottom for marker space */}
