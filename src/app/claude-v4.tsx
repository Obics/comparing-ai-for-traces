...
const OpenTelemetryTraceViewer = () => {
  ...
  const renderSpan = (span, level = 0) => {
    ...
    return (
      <React.Fragment key={span.span_id}>
        <tr 
          className={`border-b border-gray-200 transition-all duration-200 ${isSelected ? 'outline outline-2 outline-blue-500' : ''}`}
          onClick={() => setSelectedSpanId(span.span_id)}
          tabIndex={0}
          data-span-id={span.span_id}
          style={{ backgroundColor: isSelected ? 'rgba(59, 130, 246, 0.05)' : '' }}
        >
          <td className="p-2">
              ...
              <div style={{ marginLeft: `${level * 20}px` }} className="flex items-center">
                {hasChildren && (
                  <button
                    ...
                    className="mr-2 w-5 h-5 text-gray-500 flex items-center justify-center rounded hover:bg-gray-100 transition-all"
          ...
          <td className="p-2 relative">
            <div className="h-6 w-full bg-gray-100 relative">
              <div 
                className="absolute h-6 bg-blue-500 opacity-70 transition-all duration-300"
                style={{ 
                  left: `${leftPosition}%`, 
        ...
        {/* Children - with proper transitions that don't leave white space */}
        {hasChildren && (
          <tr className={`${isExpanded ? 'block' : 'hidden'}`}>
...
export default OpenTelemetryTraceViewer;